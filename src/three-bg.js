...
/* ══════════════════════════════════════════════
   3. PROBLEM — fractured shards floating apart
   ══════════════════════════════════════════════ */
export function initProblemScene(canvas) {
  if (!canvas) return null
  ...
  function animate() {
    ...
    shards.forEach(({ mesh, driftDir, rotSpeed, phase }) => {
      mesh.position.x += driftDir.x + Math.sin(t * 0.3 + phase) * 0.001
      mesh.position.y += driftDir.y + Math.cos(t * 0.4 + phase) * 0.001
      mesh.position.z += driftDir.z
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener('resize', resizeHandler)
      renderer.dispose()
      shards.forEach(({ geom, mat }) => { geom.dispose(); mat.dispose() })
    },
  }
}

/* ══════════════════════════════════════════════
   4. PLATFORM — stacked translucent layers
   ══════════════════════════════════════════════ */
export function initPlatformScene(canvas) {
  if (!canvas) return null
  ...
  const layerCount = 6
  const layerColors = [0xf59e0b, 0xfbbf24, 0xf97316, 0x4f8aff, 0x38bdf8, 0xa78bfa]
  const layers = []

  for (let i = 0; i < layerCount; i++) {
    const geom = new THREE.PlaneGeometry(3.2, 2, 16, 16)
    const edgeGeom = new THREE.EdgesGeometry(new THREE.PlaneGeometry(3.2, 2))
    ...
    const yPos = (i - (layerCount - 1) / 2) * 0.55
    ...
    group.add(plane)
    group.add(edges)
    layers.push({ plane, edges, geom, edgeGeom, mat, edgeMat, baseY: yPos, idx: i })
  }

  // data flow particles between layers
  const dpCount = isMobile() ? 20 : 50
  const dpPositions = new Float32Array(dpCount * 3)
  const dpPhases = new Float32Array(dpCount)
  const dpSpeeds = new Float32Array(dpCount)
  for (let i = 0; i < dpCount; i++) {
    dpPositions[i * 3] = (Math.random() - 0.5) * 3
    dpPositions[i * 3 + 1] = (Math.random() - 0.5) * 3.5
    dpPositions[i * 3 + 2] = (Math.random() - 0.5) * 2
    dpPhases[i] = Math.random() * Math.PI * 2
    dpSpeeds[i] = 0.3 + Math.random() * 0.5
  }
  const dpGeom = new THREE.BufferGeometry()
  dpGeom.setAttribute('position', new THREE.BufferAttribute(dpPositions, 3))
  ...
  const dpPoints = new THREE.Points(dpGeom, dpMat)
  group.add(dpPoints)
  ...
  function animate() {
    ...
    group.rotation.y = Math.sin(t * 0.12) * 0.3 + 0.3

    layers.forEach(({ plane, edges, mat, edgeMat, baseY, idx }) => {
      const hover = Math.sin(t * 0.5 + idx * 0.8) * 0.08
      plane.position.y = baseY + hover
      edges.position.y = baseY + hover
      mat.opacity = 0.05 + Math.sin(t * 0.6 + idx * 1.2) * 0.03
      edgeMat.opacity = 0.2 + Math.sin(t * 0.4 + idx) * 0.08
    })

    const pp = dpGeom.attributes.position.array
    for (let i = 0; i < dpCount; i++) {
      pp[i * 3 + 1] += dpSpeeds[i] * 0.008
      if (pp[i * 3 + 1] > 2) pp[i * 3 + 1] = -2
      pp[i * 3] += Math.sin(t + dpPhases[i]) * 0.002
    }
    dpGeom.attributes.position.needsUpdate = true
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener('resize', resizeHandler)
      renderer.dispose()
      layers.forEach(({ geom, edgeGeom, mat, edgeMat }) => {
        geom.dispose(); edgeGeom.dispose(); mat.dispose(); edgeMat.dispose()
      })
      dpGeom.dispose(); dpMat.dispose()
    },
  }
}

/* ══════════════════════════════════════════════
   5. HOW IT WORKS — flowing pipeline / data stream
   ══════════════════════════════════════════════ */
export function initHowScene(canvas) {
  if (!canvas) return null
  ...
  const tubeGeom = new THREE.TubeGeometry(curve, 200, 0.02, 8, false)
  ...
  const curvePoints2 = []
  for (let i = 0; i < steps; i++) {
    const t = (i / steps) * Math.PI * 4 + Math.PI
    curvePoints2.push(new THREE.Vector3(
      Math.sin(t) * 2.5,
      (i / steps) * 6 - 3,
      Math.cos(t) * 2.5
    ))
  ...
  const tubeGeom2 = new THREE.TubeGeometry(curve2, 200, 0.02, 8, false)
  ...
  const tube2 = new THREE.Mesh(tubeGeom2, tubeMat2)
  group.add(tube2)

  // data packets flowing along the curves
  const packetCount = isMobile() ? 15 : 30
  const packets = []
  for (let i = 0; i < packetCount; i++) {
    const geom = new THREE.SphereGeometry(0.06, 8, 8)
    const useCurve2 = i % 2 === 1
    const mat = new THREE.MeshBasicMaterial({
      color: useCurve2 ? 0x4f8aff : 0xfbbf24,
      transparent: true,
      opacity: 0.6,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
    ...
    packets.push({
      mesh, geom, mat,
      curve: useCurve2 ? curve2 : curve,
      offset: i / packetCount,
      speed: 0.04 + Math.random() * 0.03,
    })
  }

  // node markers along the first curve
  const nodeCount = 8
  for (let i = 0; i < nodeCount; i++) {
    const t = (i + 0.5) / nodeCount
    const pos = curve.getPointAt(t)
    const ringGeom = new THREE.TorusGeometry(0.2, 0.01, 8, 32)
  ...
  function animate() {
    ...
    packets.forEach((p) => {
      p.offset = (p.offset + p.speed * 0.01) % 1
      const pos = p.curve.getPointAt(p.offset)
      p.mesh.position.copy(pos)
      p.mat.opacity = 0.4 + Math.sin(t * 2 + p.offset * 10) * 0.3
    })
  ...
  return {
    ...
    destroy() {
      ...
      tubeGeom2.dispose(); tubeMat2.dispose()
      packets.forEach(({ geom, mat }) => { geom.dispose(); mat.dispose() })
    },
  }
}

/* ══════════════════════════════════════════════
   6. USE CASES — rotating globe with connection arcs
   ══════════════════════════════════════════════ */
export function initUseCasesScene(canvas) {
  if (!canvas) return null
  ...
  // wireframe globe
  const globeGeom = new THREE.SphereGeometry(1.8, 36, 24)
  ...
  for (let lat = -60; lat <= 60; lat += 30) {
    ...
    const ringGeom = new THREE.RingGeometry(r - 0.005, r + 0.005, 64)
    const ringMat = new THREE.MeshBasicMaterial({
      color: 0xf59e0b,
      transparent: true,
      opacity: 0.12,
  ...
  const arcCount = isMobile() ? 6 : 12
  const arcMeshes = []

  for (let i = 0; i < arcCount; i++) {
    const phi1 = Math.random() * Math.PI * 2
    const theta1 = Math.random() * Math.PI * 0.8 + 0.1 * Math.PI
    const phi2 = phi1 + (Math.random() - 0.5) * 3
    const theta2 = Math.random() * Math.PI * 0.8 + 0.1 * Math.PI

    const p1 = new THREE.Vector3(
      1.82 * Math.sin(theta1) * Math.cos(phi1),
      1.82 * Math.cos(theta1),
      1.82 * Math.sin(theta1) * Math.sin(phi1)
    )
    const p2 = new THREE.Vector3(
      1.82 * Math.sin(theta2) * Math.cos(phi2),
      1.82 * Math.cos(theta2),
      1.82 * Math.sin(theta2) * Math.sin(phi2)
    )

    const mid = p1.clone().add(p2).multiplyScalar(0.5)
    mid.normalize().multiplyScalar(2.5 + Math.random() * 0.8)

    const arcCurve = new THREE.QuadraticBezierCurve3(p1, mid, p2)
    const arcGeom = new THREE.TubeGeometry(arcCurve, 32, 0.008, 6, false)
    const arcMat = new THREE.MeshBasicMaterial({
      color: i % 3 === 0 ? 0x4f8aff : 0xfbbf24,
      transparent: true,
      opacity: 0.25,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
    ...
    group.add(arc)
    arcMeshes.push({ geom: arcGeom, mat: arcMat, phase: Math.random() * Math.PI * 2 })
  }
  ...
  for (let i = 0; i < dotCount; i++) {
    ...
    dotPositions[i * 3] = 1.83 * Math.sin(theta) * Math.cos(phi)
    dotPositions[i * 3 + 1] = 1.83 * Math.cos(theta)
    dotPositions[i * 3 + 2] = 1.83 * Math.sin(theta) * Math.sin(phi)
  }
  ...
  const atmoGeom = new THREE.SphereGeometry(2.1, 32, 32)
  const atmoMat = new THREE.ShaderMaterial({
    transparent: true,
    depthWrite: false,
    side: THREE.BackSide,
    blending: THREE.AdditiveBlending,
    uniforms: { uColor: { value: new THREE.Color(0xf59e0b) } },
    vertexShader: /* glsl */ `
  ...
  function animate() {
    ...
    arcMeshes.forEach(({ mat, phase }) => {
      mat.opacity = 0.15 + Math.sin(t * 0.8 + phase) * 0.12
    })
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener('resize', resizeHandler)
      renderer.dispose()
      globeGeom.dispose(); globeMat.dispose()
      arcMeshes.forEach(({ geom, mat }) => { geom.dispose(); mat.dispose() })
      dotGeom.dispose(); dotMat.dispose()
      atmoGeom.dispose(); atmoMat.dispose()
    },
  }
}

/* ══════════════════════════════════════════════
   7. CTA — orbital ring system
   ══════════════════════════════════════════════ */
